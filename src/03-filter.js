...
function getSongsBySaib(songs) {
  let saibSongs = songs.filter((song) => {return song.artist === 'Saib'})
  return saibSongs
}
...
function getSongsOverThreeMinutes(songs) {
  let threeMinSongs = songs.filter((song) => {return song.runtimeInSeconds > 180})
...
function getTitleTracks(songs) {
  let titleAlbums = songs.filter((song) => {return song.title === song.album})
  return titleAlbums 
}
